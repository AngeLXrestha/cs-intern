...
if(isset($_POST['submit'])){
	...
	$query3="UPDATE `content` 
	SET 
	`contentTitleName` = '$contentTitleName', 
	`contentTitleSlug` = '$contentTitleSlug', 
	`categoryName` = '$categoryName', 
	`image` = '$image', 
	`contentUpdateDate` = '$currentDate', 
	`contentDescription` = '$contentDescription', 
	`contentMetaTitle` = '$contentMetaTitle', 
	`contentMetaKeyword` = '$contentMetaKeyword', 
	`contentMetaDescription` = '$contentMetaDescription', 
	`content` = '$content' 
	WHERE 
	`content`.`id` = '$id';";
	$rs3=mysqli_query($cn, $query3) or die(mysqli_error($cn));

	header('Location: content.php?msg=update');
}
...
<form method="POST">
	<table>
		<tr>
			<td>Content Title</td>
			<td><input type = "text" name = "contentTitleName" value = "<?php echo $row2['contentTitleName']?>"></td>
		</tr>
		...
		<tr>
			...
			<td><textarea class = "summernote" name = "contentDescription"><?php echo $row2['contentDescription']?></textarea></td>
		</tr>

		<tr>
			<td>Content</td>
			<td><textarea class = "summernote" name = "content" value = "<?php echo $row2['content']?>"></textarea></td>
		</tr>

		<tr>
			<td>Content Meta Title</td>
			<td><input type = "text" name = "contentMetaTitle" value = "<?php echo $row2['contentMetaTitle']?>"></td>	
		</tr>

		<tr>
			<td>Content Meta Keyword</td>
			<td><input type = "text" name = "contentMetaKeyword" value = "<?php echo $row2['contentMetaKeyword']?>"></td>	
		</tr>

		<tr>
			<td>Content Meta Description</td>
			<td><input type = "text" name = "contentMetaDescription" value = "<?php echo $row2['contentMetaDescription']?>"></td>	
		</tr>
